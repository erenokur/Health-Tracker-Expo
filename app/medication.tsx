...
export default function MedicationScreen() {
  ...
  function handleSave() {
    ...
    Alert.alert("Başarılı", editing ? `${name} güncellendi.` : `${name} başarıyla eklendi.`);
  ...
  return (
    <SafeAreaView style={styles.flex} edges={["bottom"]}>
      <ScrollView contentContainerStyle={styles.container}>
        <Text style={styles.title}>{editing ? "İlacı Düzenle" : "Yeni İlaç Ekle"}</Text>

      <TextInput style={styles.input} placeholder="İlaç Adı" value={name} onChangeText={setName} />
      ...
      <Picker selectedValue={status} onValueChange={(v) => setStatus(v as ActiveStatus)} style={styles.picker}>
        <Picker.Item label="Aktif" value="Aktif" />
        <Picker.Item label="Pasif" value="Pasif" />
      </Picker>

      <Text style={styles.label}>Açlık/Tokluk</Text>
      <Picker selectedValue={mealType} onValueChange={(v) => setMealType(v as MealType)} style={styles.picker}>
      ...
      />
      <TextInput style={styles.input} placeholder="Özel Notlar" value={notes} onChangeText={setNotes} />
      <TextInput
      ...
      />
        ...
        <MenuButton label={editing ? "Güncelle" : "Kaydet"} variant="primary" onPress={handleSave} />
        <MenuButton label="İptal / Geri" variant="muted" onPress={() => router.back()} />
      </ScrollView>
    </SafeAreaView>
  );
}
